...
class AdminActions {
    ...
    async saveAllChanges() {
        ...
        } catch (e) {this.showToast(e.message || 'Erreur lors de la sauvegarde', 'error');
            if (floatingBtn) {
                floatingBtn.classList.remove('saving');
    ...
    async handleFormSubmit(form) {
        const formId = form.id || 'unknown';
        const action = form.querySelector('input[name="action"]')?.value;try {
    ...
    handleAction(action, element) {switch (action) {
            case 'delete-member':
    ...
    deleteProposal(id) {// Confirmer la suppression
    ...
    editProposal(id) {// Charger les données de la proposition
        // et ouvrir le modal d'édition
        AdminModal.open('editProposalModal', { id: id });
    }
    
    addProposal() {AdminModal.open('addProposalModal');
    }
