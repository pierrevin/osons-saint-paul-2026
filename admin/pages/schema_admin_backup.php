...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    switch ($action) {
        ...
        case 'update_equipe_section':
            ...
            if (save_json_data('site_content.json', $content)) {
                $_SESSION['success_message'] = 'Section Équipe mise à jour avec succès';
            } else {
        ...
        case 'update_event':
            ...
            if ($event_id) {
                ...
                foreach ($content['rencontres']['events'] as &$event) {
                ...
                $content['rencontres']['events'][] = $new_event;
            }
        ...
        case 'update_principle':
            ...
            if (save_json_data('site_content.json', $content)) {
                $_SESSION['success_message'] = 'Principe ajouté avec succès';
            } else {
                $_SESSION['error_message'] = 'Erreur lors de la sauvegarde';
            }
            header('Location: ' . $_SERVER['REQUEST_URI']);
        ...
        <main class="admin-main">
            <div class="schema-container">
                ...
                <div class="schema-header">
                    <h1><i class="fas fa-th-large"></i> Vue Schématique du Site</h1>
                    <p>Gérez le contenu de votre site en un coup d'œil</p>
                </div>

                <!-- Indicateurs rapides -->
                <div class="admin-indicators">
                    <div class="indicator-card" onclick="toggleSection('hero'); scrollToSection('hero-content');">
                        <div class="indicator-icon">🏠</div>
                        <div class="indicator-content">
                            <div class="indicator-title">Hero</div>
                            <div class="indicator-subtitle">Image principale</div>
                    </div>
                    </div>
                    
                    <div class="indicator-card" onclick="toggleSection('programme'); scrollToSection('programme-content');">
                        <div class="indicator-icon">📋</div>
                        <div class="indicator-content">
                            <div class="indicator-title">Programme</div>
                            <div class="indicator-subtitle"><?= count($content['programme']['proposals'] ?? []) ?> propositions</div>
                    </div>
                    </div>
                    
                    <div class="indicator-card" onclick="toggleSection('equipe'); scrollToSection('equipe-content');">
                        <div class="indicator-icon">👥</div>
                        <div class="indicator-content">
                            <div class="indicator-title">Équipe</div>
                            <div class="indicator-subtitle"><?= count($content['equipe']['members'] ?? []) ?> membres</div>
                    </div>
                </div>
                    
                    <div class="indicator-card" onclick="toggleSection('rendez_vous'); scrollToSection('rendez_vous-content');">
                        <div class="indicator-icon">📅</div>
                        <div class="indicator-content">
                            <div class="indicator-title">Rencontres</div>
                            <div class="indicator-subtitle"><?= count($content['rencontres']['events'] ?? []) ?> événements</div>
                        </div>
                    </div>
                    
                    <div class="indicator-card" onclick="toggleSection('charte'); scrollToSection('charte-content');">
                        <div class="indicator-icon">📜</div>
                        <div class="indicator-content">
                            <div class="indicator-title">Charte</div>
                            <div class="indicator-subtitle">Valeurs</div>
                        </div>
                    </div>
                    
                    <div class="indicator-card" onclick="toggleSection('idees'); scrollToSection('idees-content');">
                        <div class="indicator-icon">💡</div>
                        <div class="indicator-content">
                            <div class="indicator-title">Idées</div>
                            <div class="indicator-subtitle"><?= count($content['idees']['ideas'] ?? []) ?> idées</div>
                        </div>
                    </div>
                    
                    <div class="indicator-card" onclick="toggleSection('mediatheque'); scrollToSection('mediatheque-content');">
                        <div class="indicator-icon">📚</div>
                        <div class="indicator-content">
                            <div class="indicator-title">Médiathèque</div>
                            <div class="indicator-subtitle">Ressources</div>
                        </div>
                    </div>
                    
                    <div class="indicator-card" onclick="switchTab('citizen-proposals')">
                        <div class="indicator-icon">💡</div>
                        <div class="indicator-content">
                            <div class="indicator-title">Propositions</div>
                            <div class="indicator-subtitle">
                                <span id="citizen-total-count">0</span> total<br>
                                <span id="citizen-pending-count" style="color: #ffc107;">0</span> à traiter, 
                                <span id="citizen-approved-count" style="color: #28a745;">0</span> validées, 
                                <span id="citizen-rejected-count" style="color: #dc3545;">0</span> rejetées
                            </div>
                        </div>
                    </div>
                    
                    <a href="propositions-analytics.php" target="_blank" class="indicator-card">
                        <div class="indicator-icon">📊</div>
                        <div class="indicator-content">
                            <div class="indicator-title">Analyse</div>
                            <div class="indicator-subtitle">Graphiques & statistiques</div>
                        </div>
                    </a>
                </div>
    ...
    <div id="addMemberModal" class="modal-overlay" style="display: none;">
        <div class="modal-container">
            <div class="modal-header">
                ...
                <button onclick="closeModal('addMemberModal')" class="btn-close">&times;</button>
            </div>
            <div class="modal-body">
    ...
    <script>
        ...
        function editMember(id) {
            ...
            if (member) {
                ...
                document.getElementById('edit-member-image').value = member.image || '';
        ...
        function getEventById(id) {
            // Récupérer l'événement depuis les données PHP
            const events = <?= json_encode($content['rencontres']['events'] ?? []) ?>;
        ...
        function saveEvent() {
            const form = document.getElementById('add-event-form') || document.getElementById('edit-event-form');
            if (!form) {
            ...
            formData.append('section', 'rencontres');
            formData.append('csrf_token', '<?= generate_csrf_token() ?>');
        ...
        function savePrinciple() {
            ...
            .then(data => {
                ...
                showToast('Principe ajouté avec succès', 'success');
            })
            .catch(error => {
                console.error('Erreur lors de la sauvegarde:', error);
                showToast('Erreur lors de la sauvegarde', 'error');
    ...
    <div id="editMemberModal" class="modal-overlay" style="display: none;">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button onclick="closeModal('editMemberModal')" class="btn-close">&times;</button>
            </div>
            <div class="modal-body">
                <form id="edit-member-form">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="edit-member-description" rows="3"></textarea>
                    </div>
                    
                    <div class="form-group">
                        <label for="edit-member-image">Image du membre</label>
                        <input type="file" id="edit-member-image" accept="image/*">
            ...
            <div class="modal-body">
                <form id="add-event-form">
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="add-event-description" rows="3"></textarea>
                    </div>
                    
                    <div class="form-group">
                        <label for="add-event-date">Date *</label>
                        <input type="datetime-local" id="add-event-date" required>
                    </div>
