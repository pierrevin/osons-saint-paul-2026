...
document.addEventListener('DOMContentLoaded', function() {
    // Parallax simple pour faire arriver le titre depuis le haut et l'arrêter au-dessus du bas
    ...
    if (hero && heroContent) {
        const onScroll = () => {
            ...
            // ratio de visibilité du hero dans la fenêtre
            let ratio = 1 - (rect.top / viewportH);
            ratio = Math.max(0, Math.min(1, ratio));
            // Parallax: titre descend de 14vh -> 0vh
            const shift = (1 - ratio) * 14; // vh
            heroContent.style.setProperty('--title-shift', shift + 'vh');
            // Laisser l'opacité à 1 pour éviter les problèmes de visibilité
            heroContent.style.opacity = 1;
        };
        window.addEventListener('scroll', onScroll, { passive: true });
        onScroll();
    }
    // Dissiper le flou du hero après un léger scroll
    const bodyEl = document.body;
    const blurToggle = () => {
        if (window.scrollY > window.innerHeight * 0.12) {
            bodyEl.classList.add('hero-blur-off');
        } else {
            bodyEl.classList.remove('hero-blur-off');
        }
    };
    window.addEventListener('scroll', blurToggle, { passive: true });
    blurToggle();
    // Logo scroll-down entre hero et programme
    const divider = document.querySelector('.section-divider-logo');
    if (divider) {
        const scrollNext = () => {
            const nextSection = document.querySelector('#programme, .programme, section.programme');
            if (nextSection) nextSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
        };
        divider.addEventListener('click', (e) => { e.preventDefault(); scrollNext(); });
        divider.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); scrollNext(); }
        });
    }
    // Scroll au clic sur le logo hero
    const heroLogo = document.querySelector('.hero-logo');
    if (heroLogo) {
        heroLogo.addEventListener('click', function(e) {
            e.preventDefault();
            const nextSection = document.querySelector('.programme, section.programme');
            if (nextSection) {
        ...
        heroLogo.setAttribute('role', 'button');
        heroLogo.setAttribute('tabindex', '0');
        heroLogo.setAttribute('aria-label', 'Voir la suite');
    ...
    // Scroll via séparateur logo entre sections (éviter redéclaration)
    const divider2 = document.querySelector('.section-divider-logo');
    if (divider2) {
        const scrollNext = () => {
            const nextSection = document.querySelector('#programme, .programme, section.programme');
            if (nextSection) nextSection.scrollIntoView({ behavior: 'smooth', block: 'start' });
        };
        divider2.addEventListener('click', (e) => { e.preventDefault(); scrollNext(); });
        divider2.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); scrollNext(); }
        });
    }
    // Navigation smooth scroll (délégation unique)
    document.addEventListener('click', function(e) {
        ...
        // Lien Infolettre: ancre native vers #newsletter-cta
        if (href === '#newsletter-cta') {
            // Fermer le menu mobile si ouvert
            const headerNav = document.querySelector('.header-nav');
            const mobileMenuToggle = document.getElementById('mobileMenuToggle');
            if (headerNav && headerNav.classList.contains('mobile-active')) {
                setTimeout(() => {
        ...
        // Liens internes seulement
        if (href.startsWith('#')) {
            e.preventDefault();
            const targetSection = document.querySelector(href);
            if (!targetSection) return;
            
            // Smooth scroll + compensation header sticky
            targetSection.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' });
            setTimeout(() => {
                const headerEl = document.getElementById('headerSticky');
...
function updateHeaderOnScroll() {
    ...
    if (!header) return;
    const threshold = heroEl ? Math.min(120, Math.max(60, heroEl.clientHeight * 0.1)) : 100;
...
// ===== SCROLLSPY - SUPPRIMÉ (doublon). Utiliser updateActiveLink() plus bas =====
...
// ===== ACCESSIBILITÉ =====
document.addEventListener('DOMContentLoaded', function() {
    // Gestion du focus pour l'accessibilité (exclure les liens du menu .nav-link)
    const focusableElements = document.querySelectorAll('button, input, textarea, select, a.btn, .proposition-card');
...
function initializeHeaderSticky() {
    ...
    // Navigation smooth scroll - SUPPRIMÉ (gestionnaire dupliqué)
...
function initializeMobileMenu() {
    ...
    if (mobileMenuToggle && headerNav) {
        ...
        // Fermeture du menu déléguée au gestionnaire global de navigation ci-dessus
    }
}
